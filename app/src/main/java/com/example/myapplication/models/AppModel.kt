...
import com.example.myapplication.constants.CellConstans
import com.example.myapplication.constants.FieldConstants
...
class AppModel {
    ...
    private fun boostScore() {
        ...
        if (score > preferences?.getHighScore() as Int)
            preferences?.saveHighScope(score)
    }
        ...
        return if (position.y < 0 || position.x < 0) {
            ...
            for (i in 0 until shape.size) {
                for (j in 0 until shape[i].size) {
                    val y = position.y + i
                    val x = position.x + j
                    if (CellConstans.EMPTY.value != shape[i][j]
                        && CellConstans.EMPTY.value != field[y][x]
                    ) {
                        return false
                    }
                }
    ...
    fun generateField(action: String) {
        ...
        private fun resetField(ephemeralCellsOnly: Boolean = true) {
            for (i in 0 until FieldConstants.ROW_COUNT.value) {
                (0 until FieldConstants.COLUMN_COUNT.value)
                    .filter { !ephemeralCellsOnly || field[i][it] == CellConstans.EPHEMERAL.value }
                    .forEach { field[i][it] = CellConstans.EMPTY.value }
            }
        }
        ...
        private fun persistCellData() {
            for (i in 0 until field.size) {
                for (j in 0 until field[i].size) {
                    ...
                    if (status == CellConstans.EPHEMERAL.value) {
        ...
        private fun assesField() {
            for (i in 0 until field.size) {
                ...
                for (j in 0 until field[i].size) {
                    ...
                    val isEmpty = CellConstans.EMPTY.value == status
                    if (isEmpty)
                        emptyCells++
                }
                if (emptyCells == 0)
        ...
        private fun shiftRows(nToRow: Int) {
            ...
            for (j in 0 until field[0].size)
                setCellStatus(0, j, CellConstans.EMPTY.value)
        }
