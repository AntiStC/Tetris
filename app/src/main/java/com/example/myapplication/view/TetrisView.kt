...
import com.example.myapplication.models.AppModel
import com.example.myapplication.models.Block
...
class TetrisView : View {
    ...
    private fun drawCell(canvas: Canvas, row: Int, col: Int) {
        ...
        if (CellConstans.EMPTY.value != cellStatus) {
            val color = if (CellConstans.EPHEMERAL.value == cellStatus) {
    ...
    override fun onSizeChanged(width: Int, height: Int, previousWidth: Int, previousHeight: Int) {
        ...
        val callHeight = (height - 2 * FRAME_OFFSET_BASE) / FieldConstants.ROW_COUNT.value
        val n = Math.min(cellWidth, callHeight)
